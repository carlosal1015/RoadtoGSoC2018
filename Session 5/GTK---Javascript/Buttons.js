...
const MyWindow = new Lang.Class({
    ...
    _init: function() {
        this.parent({title:"Esta es la pantalla de la sesion 3 con Javascript"});

        this.box = new Gtk.Box({spacing: 6});
        this.add(this.box);

        this.button1 = new Gtk.Button({label: "Javascrip1"});
        this.button1.connect("clicked", this.onButton1Clicked);
        this.box.pack_start(this.button1, true, true, 0);

        this.button2 = new Gtk.Button({label: "Javascript2"});
